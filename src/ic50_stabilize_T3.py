...
DATA_PATH = os.environ.get(
    "DATA_PATH",
    "/home/gna/workspase/education/MEPHI/Coursework-Classical_ml/data/data.xlsx",
)
    ...
    for name, model in models:
        ...
        for train_idx, test_idx in kf.split(X_full):
            ...
            X_train_s = pd.DataFrame(
                scaler.fit_transform(X_train_imp), columns=X_train.columns
            )
            X_test_s = pd.DataFrame(
                scaler.transform(X_test_imp), columns=X_test.columns
            )
            # fit on log target
        ...
        X_s_full = pd.DataFrame(
            scaler.fit_transform(X_imp_full), columns=X_full.columns
        )
        ...
        joblib.dump(
            {"imputer": imp, "scaler": scaler, "model": model, "cap": cap, "pctl": p},
            ART / art_name,
        )
...
for r in results_summary:
    ...
    rows.append(
        {
            "model": f"{r['model']}_capped_{p_int}",
            "rmse_mean": r["rmse_mean"],
            "mae_mean": r["mae_mean"],
            "r2_mean": r["r2_mean"],
            "notes": f"capped_{r['pctl']}",
        }
    )
...
with open(report_md, "w") as f:
    ...
    f.write(
        f"Dropped {dropped_target_rows} rows with NaN in target IC50 (from {before_count} to {after_count}).\n\n"
    )
    f.write("Winsorization percentiles tested: 99.0, 99.5, 99.9\n\n")
    f.write("Results (RMSE / MAE / R2):\n")
    for r in results_summary:
        f.write(
            f"- pctl={r['pctl']}: {r['model']}: RMSE={r['rmse_mean']:.4f} (std {r['rmse_std']:.4f}), MAE={r['mae_mean']:.4f}, R2={r['r2_mean']:.4f}\n"
        )
