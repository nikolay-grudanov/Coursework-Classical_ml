...
def clean_data(data: pd.DataFrame) -> pd.DataFrame:
    ...
    # Remove exact duplicate rows early to avoid leakage into splits
    dup_count = int(data.duplicated().sum())
    if dup_count > 0:
        logger.info(
            f"Found {dup_count} exact duplicate rows; dropping duplicates (keep=first)"
        )
...
def split_data(data: pd.DataFrame, test_size: float = 0.2, random_state: int = 42):
    ...
    _, _, train_si_8_class, test_si_8_class = train_test_split(
        ...
        test_size=test_size,
        random_state=random_state,
        stratify=target_si_8_class,
    )

    logger.info(
        f"Split data into train ({len(train_features)} samples) and test ({len(test_features)} samples) sets"
    )
