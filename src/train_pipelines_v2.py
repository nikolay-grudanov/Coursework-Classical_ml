...
def run_regression_cv(X, y, model, n_splits=5, random_state=42):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X), start=1):
        ...
        X_train_scaled = pd.DataFrame(
            scaler.fit_transform(X_train_imp), columns=X.columns
        )
    ...
    summary = {
        k + ("_mean" if k != "per_fold" else ""): (
            float(np.mean(v)) if isinstance(v, list) else v
        )
...
def main(data_path: str, artifact_dir: str):
    ...
    missing = [
        n
        for n, v in [("IC50", ic50_col), ("CC50", cc50_col), ("SI", si_col)]
        if v is None
    ]
    if missing:
