"""python src/train_pipelines.py

Train regression and classification pipelines per coursework instructions.

Preprocessing:
- Regression: SimpleImputer(median) -> StandardScaler -> model
- Classification: (optional) apply SMOTE inside train folds -> StandardScaler -> model

Models tried:
- IC50 regression: LinearRegression, RandomForestRegressor, LightGBM (if installed)
- SI>=8 classification: LogisticRegression(class_weight='balanced'), GradientBoosting, LightGBM (if installed)

Validation:
- 5-fold CV; StratifiedKFold for classification

Logging:
- Fold metrics and averages saved to artifacts/training_report.json
- Model artifacts saved to artifacts/{task}_{model_name}.joblib
...
def run_regression_cv(
    X, y, model, n_splits=5, random_state=42, artifact_dir=Path("artifacts")
):
    ...
    for train_idx, test_idx in kf.split(X):
        ...
        X_train_scaled = pd.DataFrame(
            scaler.fit_transform(X_train_imp), columns=X.columns, index=X_train.index
        )
        X_test_scaled = pd.DataFrame(
            scaler.transform(X_test_imp), columns=X.columns, index=X_test.index
        )
        # fit model
...
def run_classification_cv(X, y, model, use_smote=False, n_splits=5, random_state=42):
    ...
    for train_idx, test_idx in skf.split(X, y):
        ...
        if use_smote:
            if not HAS_SMOTE:
                warnings.warn(
                    "SMOTE requested but imbalanced-learn not installed; continuing without SMOTE"
                )
        ...
        print(
            f"[CLF] Fold {fold} -> Acc: {acc:.4f}, Prec: {prec:.4f}, Rec: {rec:.4f}, F1: {f1:.4f}, AUC: {auc}"
        )
...
def main(data_path: str, artifact_dir: str):
    ...
    # Features: numerical descriptors excluding IC50, CC50, SI
    FEATURES = [c for c in numeric_cols if c not in ["IC50", "CC50", "SI"]]
    print(f"Using {len(FEATURES)} numeric features (excluding IC50/CC50/SI)")

    report = {"versions": versions, "tasks": {}}

    # ---- Regression: IC50 ----
    reg_df = df.dropna(subset=["IC50"]).reset_index(drop=True)
    ...
    # ---- Classification: SI >= 8 ----
    class_df = df.dropna(subset=["SI"]).reset_index(drop=True)
    X_clf = class_df[FEATURES]
    y_si8 = (class_df["SI"] >= 8).astype(int)
    ...
    for name, model in clf_models:
        ...
        # fit final pipeline on all data (apply SMOTE if available)
        imputer = SimpleImputer(strategy="median")
        scaler = StandardScaler()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data",
        default=None,
        help="Path to data file. Can be overridden by DATA_PATH env var or .env",
    )
    parser.add_argument("--artifacts", default="artifacts")
    args = parser.parse_args()
    import os

    data_arg = args.data or os.environ.get("DATA_PATH") or "data/data.xlsx"
    main(data_arg, args.artifacts)
