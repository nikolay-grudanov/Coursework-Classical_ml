...
def run_regression_cv(X, y, model, n_splits=5, random_state=42, artifact_dir=Path('artifacts')):
...
def run_classification_cv(X, y, model, use_smote=False, n_splits=5, random_state=42):
    ...
    for train_idx, test_idx in skf.split(X, y):
        ...
        if use_smote:
            if not HAS_SMOTE:
                warnings.warn('SMOTE requested but imbalanced-learn not installed; continuing without SMOTE')
